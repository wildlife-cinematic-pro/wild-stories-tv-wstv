      ...
      <ControlStack>
        ...
        <WorkspaceCard title="Location preset" description="Choose the exact scenic preset that drives the prompts.">
          <select
            ...
            className="w-full min-w-0 rounded-2xl border border-white/[0.08] bg-gray-950 px-3 py-2 text-sm font-semibold text-white outline-none"
          >
          ...
          <div className="mt-3 min-w-0 rounded-2xl border border-cyan-300/10 bg-cyan-300/[0.04] p-3 text-xs leading-5 text-white/60 [overflow-wrap:anywhere]">
      ...
      <ControlStack>
        ...
        <WorkspaceCard title="Prompt strength" description="Keep the output compact on screen while preserving the same full-copy prompt text underneath.">
          ...
          <textarea
            ...
            className="mt-2 min-h-[120px] w-full min-w-0 rounded-2xl border border-white/[0.08] bg-gray-950 px-3 py-2 text-sm text-white outline-none placeholder:text-white/25"
          />
        </WorkspaceCard>
      </ControlStack>
    );
  ...
  return (
    <WorkspaceCard title="Quick random" description="Jump to a new preset without touching the underlying output logic.">
      <div className="grid min-w-0 gap-2 sm:grid-cols-2 xl:grid-cols-1">
        <button type="button" onClick={randomUSA} className="min-h-10 min-w-0 rounded-2xl border border-cyan-300/20 bg-cyan-300/[0.06] px-3 py-2 text-xs font-black leading-snug text-cyan-100 transition [overflow-wrap:anywhere] hover:bg-cyan-300/[0.1]">Random USA Featured</button>
        <button type="button" onClick={randomUSAMore} className="min-h-10 min-w-0 rounded-2xl border border-cyan-300/20 bg-cyan-300/[0.06] px-3 py-2 text-xs font-black leading-snug text-cyan-100 transition [overflow-wrap:anywhere] hover:bg-cyan-300/[0.1]">Random USA More Parks</button>
        <button type="button" onClick={randomCanada} className="min-h-10 min-w-0 rounded-2xl border border-emerald-300/20 bg-emerald-300/[0.06] px-3 py-2 text-xs font-black leading-snug text-emerald-100 transition [overflow-wrap:anywhere] hover:bg-emerald-300/[0.1]">Random Canada</button>
        <button type="button" onClick={randomJapan} className="min-h-10 min-w-0 rounded-2xl border border-rose-300/20 bg-rose-300/[0.06] px-3 py-2 text-xs font-black leading-snug text-rose-100 transition [overflow-wrap:anywhere] hover:bg-rose-300/[0.1]">Random Japan</button>
        <button type="button" onClick={randomEurope} className="min-h-10 min-w-0 rounded-2xl border border-violet-300/20 bg-violet-300/[0.06] px-3 py-2 text-xs font-black leading-snug text-violet-100 transition [overflow-wrap:anywhere] hover:bg-violet-300/[0.1]">Random Europe</button>
        <button type="button" onClick={randomWorld} className="min-h-10 min-w-0 rounded-2xl border border-lime-300/20 bg-lime-300/[0.06] px-3 py-2 text-xs font-black leading-snug text-lime-100 transition [overflow-wrap:anywhere] hover:bg-lime-300/[0.1]">Random World</button>
        <button type="button" onClick={randomViral} className="min-h-10 min-w-0 rounded-2xl border border-amber-300/20 bg-amber-300/[0.08] px-3 py-2 text-xs font-black leading-snug text-amber-100 transition [overflow-wrap:anywhere] hover:bg-amber-300/[0.12]">Random best viral preset</button>
      </div>
    </WorkspaceCard>
  );
}
