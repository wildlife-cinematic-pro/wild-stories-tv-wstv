...
describe("hybrid reference image prompts", () => {
  ...
  it("keeps the final merge prompt image-focused with Runway reference tags", () => {
    ...
    expect(prompt).toContain("Preserve Crocodile identity from LEAD_REF");
    expect(prompt).toContain("Preserve Warthog identity from OPPOSITE_REF");
    expect(prompt).toContain("atmosphere from ENV_REF");
